...
impl SysDictDataService {
    ...
    pub async fn detail(&self, dict_code: &str) -> Result<SysDictDataVO> {
        let dict_data = SysDictData::select_by_map(pool!(), rbs::value! {"dict_code": dict_code})
            .await?
            .into_iter()
            .next()
            .ok_or_else(|| Error::from(format!("不存在:{:?} ", dict_code)))?;
        let dict_data_vo = SysDictDataVO::from(dict_data);
        Ok(dict_data_vo)
    }

    pub async fn add(&self, arg: &SysDictData) -> Result<u64> {
        self.check_dict_value_unique(
            &None,
            &arg.dict_type.clone().unwrap_or_default(),
            &arg.dict_value.clone().unwrap_or_default(),
        )
        .await?;
    ...
    pub async fn update(&self, data: SysDictData) -> Result<u64> {
        self.check_dict_value_unique(
            &data.dict_code.clone(),
            &data.dict_type.clone().unwrap_or_default(),
            &data.dict_value.clone().unwrap_or_default(),
        )
        .await?;
        let result =
            SysDictData::update_by_map(pool!(), &data, rbs::value! {"dict_code": data.dict_code.clone()}).await;
        if result.is_ok() {
            self.update_cache().await?;
        }
    ...
    pub async fn remove(&self, dict_code: &str) -> Result<u64> {
        let targets = SysDictData::select_by_map(pool!(), rbs::value! {"dict_code": dict_code}).await?;

        let r = SysDictData::delete_by_map(pool!(), rbs::value! {"dict_code": dict_code}).await?;
    ...
    pub async fn update_cache(&self) -> Result<()> {
        let mut all = SysDictData::select_by_map(
            pool!(),
            rbs::value! {"column":vec!["dict_type","dict_label","dict_value","css_class","list_class","dict_sort"]},
        )
        .await?;
